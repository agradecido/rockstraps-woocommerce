...
class WXRImporter extends \WP_Importer {
	...
	public function __construct( $options = array() ) {
		...
		$this->options = wp_parse_args(
			...
			array(
				...
				'prefill_existing_comments' => false,
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', esc_html__( 'Could not open the file for parsing', 'vignette' ) );
		}
		return $reader;
	}
	/**
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', esc_html__( 'The file does not exist, please try again.', 'vignette' ) );
		}
			...
			switch ( $child->tagName ) {
				...
				case 'wp:status':
					...
					if ( 'auto-draft' === $data['post_status'] ) {
						// Bail now.
						return new WP_Error( 'wxr_importer.post.cannot_import_draft', __( 'Cannot import auto-draft posts', 'vignette' ), $data );
					}
					break;
				case 'wp:post_parent':
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', esc_html__( 'Invalid file type', 'vignette' ) );
		}
	 ...
	 * Callback for `usort` to sort comments by ID
	 *
	 * @param array $node Comment data for the first comment.
	 *
	 * @return int
	 */
	...
	protected function parse_term_node( $node, $type = 'term' ) {
		...
		foreach ( $node->childNodes as $child ) {
			...
			} else if ( 'wp:termmeta' == $child->tagName ) {
				$meta_item = $this->parse_meta_node( $child );
				if ( ! empty( $meta_item ) ) {
					$meta[] = $meta_item;
				}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( 200 !== $code ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					// translators: remote server string.
					__( 'Remote server returned %1$d %2$s for %3$s', 'vignette' ),
					$code,
		...
		// unlink( $upload['file'] );
		// return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'vignette' ) );
		// }.
		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'vignette' ) );
		}
		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			// translators: remote file string.
			$message = sprintf( esc_html__( 'Remote file is too large, limit is %s', 'vignette' ), size_format( $max_size ) );
	 ...
	 * Post process posts.
	 *
	 * @param var $todo importing parents.
	 */
	 ...
	 * Post process posts.
	 *
	 * @param var $post_id for menu items.
	 */
	...
	protected function prefill_existing_terms() {
		global $wpdb;
		$terms = $wpdb->prepare(
			'SELECT %s FROM %s AS %s JOIN %s AS %s ON %s',
