...
class Merlin {
	...
	public function __construct( $config = array(), $strings = array() ) {
		...
		$strings = array(
			'admin-menu'               => esc_html__( 'Theme Setup', 'vignette' ),
			/* translators: 1: Title Tag 2: Theme Name 3: Closing Title Tag */
			'title%s%s%s%s'            => esc_html__( '%1$s%2$s Themes &lsaquo; Theme Setup: %3$s%4$s', 'vignette' ),
			'return-to-dashboard'      => esc_html__( 'Return to the dashboard', 'vignette' ),
			'ignore'                   => esc_html__( 'Disable this wizard', 'vignette' ),
			'btn-skip'                 => esc_html__( 'Skip', 'vignette' ),
			'btn-next'                 => esc_html__( 'Next', 'vignette' ),
			'btn-start'                => esc_html__( 'Start', 'vignette' ),
			'btn-no'                   => esc_html__( 'Cancel', 'vignette' ),
			'btn-plugins-install'      => esc_html__( 'Install', 'vignette' ),
			'btn-child-install'        => esc_html__( 'Install', 'vignette' ),
			'btn-content-install'      => esc_html__( 'Install', 'vignette' ),
			'btn-import'               => esc_html__( 'Import', 'vignette' ),
			'btn-license-activate'     => esc_html__( 'Activate', 'vignette' ),
			'btn-license-skip'         => esc_html__( 'Later', 'vignette' ),
			/* translators: Theme Name */
			'license-header%s'         => esc_html__( 'Activate %s', 'vignette' ),
			/* translators: Theme Name */
			'license-header-success%s' => esc_html__( '%s is Activated', 'vignette' ),
			/* translators: Theme Name */
			'license%s'                => esc_html__( 'Enter your license key to enable remote updates and theme support.', 'vignette' ),
			'license-label'            => esc_html__( 'License key', 'vignette' ),
			'license-success%s'        => esc_html__( 'The theme is already registered, so you can go to the next step!', 'vignette' ),
			'license-json-success%s'   => esc_html__( 'Your theme is activated! Remote updates and theme support are enabled.', 'vignette' ),
			'license-tooltip'          => esc_html__( 'Need help?', 'vignette' ),
			/* translators: Theme Name */
			'welcome-header%s'         => esc_html__( 'Welcome to %s', 'vignette' ),
			'welcome-header-success%s' => esc_html__( 'Hi. Welcome back', 'vignette' ),
			'welcome%s'                => esc_html__( 'This wizard will set up your theme, install plugins, and import content. It is optional & should take only a few minutes.', 'vignette' ),
			'welcome-success%s'        => esc_html__( 'You may have already run this theme setup wizard. If you would like to proceed anyway, click on the "Start" button below.', 'vignette' ),
			'child-header'             => esc_html__( 'Install Child Theme', 'vignette' ),
			'child-header-success'     => esc_html__( 'You\'re good to go!', 'vignette' ),
			'child'                    => esc_html__( 'Let\'s build & activate a child theme so you may easily make theme changes.', 'vignette' ),
			'child-success%s'          => esc_html__( 'Your child theme has already been installed and is now activated, if it wasn\'t already.', 'vignette' ),
			'child-action-link'        => esc_html__( 'Learn about child themes', 'vignette' ),
			'child-json-success%s'     => esc_html__( 'Awesome. Your child theme has already been installed and is now activated.', 'vignette' ),
			'child-json-already%s'     => esc_html__( 'Awesome. Your child theme has been created and is now activated.', 'vignette' ),
			'plugins-header'           => esc_html__( 'Install Plugins', 'vignette' ),
			'plugins-header-success'   => esc_html__( 'You\'re up to speed!', 'vignette' ),
			'plugins'                  => esc_html__( 'Let\'s install some essential WordPress plugins to get your site up to speed.', 'vignette' ),
			'plugins-success%s'        => esc_html__( 'The required WordPress plugins are all installed and up to date. Press "Next" to continue the setup wizard.', 'vignette' ),
			'plugins-action-link'      => esc_html__( 'Advanced', 'vignette' ),
			'import-header'            => esc_html__( 'Import Content', 'vignette' ),
			'import'                   => esc_html__( 'Let\'s import content to your website, to help you get familiar with the theme.', 'vignette' ),
			'import-action-link'       => esc_html__( 'Advanced', 'vignette' ),
			'ready-header'             => esc_html__( 'All done. Have fun!', 'vignette' ),
			/* translators: Theme Author */
			'ready%s'                  => esc_html__( 'Your theme has been all set up. Enjoy your new theme by %s.', 'vignette' ),
			'ready-action-link'        => esc_html__( 'Extras', 'vignette' ),
			'ready-big-button'         => esc_html__( 'View your website', 'vignette' ),
			'ready-link-1'             => sprintf( '<a href="%1$s" target="_blank">%2$s</a>', 'https://vivathemes.com/contact/', esc_html__( 'Get Theme Support', 'vignette' ) ),
		);
	...
	public function switch_theme() {
		if ( ! is_child_theme() ) {
			set_transient( $this->theme->template . '_merlin_redirect', 0 );
		}
	}
	...
	public function ignore() {
		...
		if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( isset( $_GET['_wpnonce'] ), 'merlinwp-ignore-nounce' ) || ! is_admin() || ! isset( $_GET[ $this->ignore ] ) || ! current_user_can( 'manage_options' ) ) ) {
			return;
		}
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'vignette' ),
		);
		...
		*/
		...
		<div class="merlin__wrapper">
			...
			<?php printf( '<a class="return-to-dashboard" href="%s">%s</a>', esc_url( admin_url( '/' ) ), esc_html( $strings['return-to-dashboard'] ) ); ?>
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'vignette' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'vignette' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name' => esc_html__( 'Welcome', 'vignette' ),
		...
		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'vignette' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'vignette' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'vignette' ),
				'view' => array( $this, 'content' ),
			);
		}
		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'vignette' ),
	...
	protected function plugins() {
		...
		$method = '';
		if ( isset( $_POST ) || wp_verify_nonce( sanitize_key( wp_unslash( $_POST ), 'wpnonce' ) ) ) {
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
										<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'vignette' ); ?>">
											<?php esc_html_e( 'req', 'vignette' ); ?>
										</span>
									</span>
								</label>
							</li>
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'vignette' ); ?>">
					...
					<div class="merlin__select-control-preview">
						<?php if ( $landing_url ) : ?>
							<a class="all-demos" href="<?php echo esc_url( $landing_url ); ?>" target="_blank"><?php esc_html_e( 'See All Demos', 'vignette' ); ?></a>
						<?php endif; ?>
						<?php if ( ! empty( $import_file['import_preview_url'] ) ) : ?>
							<a class="current-demo" href="<?php echo esc_attr( $import_file['import_preview_url'] ); ?>" target="_blank"><?php esc_html_e( 'See ', 'vignette' ); ?><span><?php echo esc_html( $this->import_files[0]['import_file_name'] ); ?></span></a>
						<?php endif; ?>
					</div>
				</div>
	 ...
	 *
	 * @param string $string Clean string.
	 */
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'vignette' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'vignette' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'vignette' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'vignette' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'vignette' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'vignette' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'vignette' );
						break;
...
";
	...
	public function generate_child_screenshot( $path ) {
		...
		} elseif ( file_exists( $this->base_path . '/screenshot.png' ) ) {
	...
	public function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2' => - 1,
					'message' => esc_html__( 'Activating', 'vignette' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2' => - 1,
					'message' => esc_html__( 'Updating', 'vignette' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2' => - 1,
					'message' => esc_html__( 'Installing', 'vignette' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			$json['message'] = esc_html__( 'Installing', 'vignette' );
			wp_send_json( $json );
			...
			wp_send_json(
				array(
					'done' => 1,
					'message' => esc_html__( 'Success', 'vignette' ),
				)
			);
		}
	...
	public function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			...
			wp_send_json_error(
				array(
					'error' => 1,
					'message' => esc_html__( 'Invalid content!', 'vignette' ),
				)
			);
		}
		...
		if ( $json ) {
			...
			wp_send_json(
				array(
					'error' => 1,
					'message' => esc_html__( 'Error', 'vignette' ),
					'logs' => '',
					'errors' => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			...
			wp_send_json_error(
				array(
					'error' => 1,
					'message' => esc_html__( 'Invalid data!', 'vignette' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title' => esc_html__( 'Content', 'vignette' ),
				'description' => esc_html__( 'Demo content data.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title' => esc_html__( 'Widgets', 'vignette' ),
				'description' => esc_html__( 'Sample widgets data.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
				'install_callback' => array( 'Merlin_Widget_Importer', 'vignette' ),
				'checked' => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title' => esc_html__( 'Revolution Slider', 'vignette' ),
				'description' => esc_html__( 'Sample Revolution sliders data.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title' => esc_html__( 'Options', 'vignette' ),
				'description' => esc_html__( 'Sample theme options data.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title' => esc_html__( 'Redux Options', 'vignette' ),
				'description' => esc_html__( 'Redux framework options.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
		...
		if ( ! empty( $import_files['option_tree'] ) ) {
			$content['option_tree'] = array(
				'title' => esc_html__( 'Option Tree Options', 'vignette' ),
				'description' => esc_html__( 'Option Tree framework options.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
		...
		if ( ! empty( $import_files['cptui'] ) ) {
			$content['cptui'] = array(
				'title' => esc_html__( 'CPTUI Options', 'vignette' ),
				'description' => esc_html__( 'CPTUI framework options.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title' => esc_html__( 'After import setup', 'vignette' ),
				'description' => esc_html__( 'After import setup.', 'vignette' ),
				'pending' => esc_html__( 'Pending', 'vignette' ),
				'installing' => esc_html__( 'Installing', 'vignette' ),
				'success' => esc_html__( 'Success', 'vignette' ),
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'vignette' );
		$data['proceed']  = 'true';
